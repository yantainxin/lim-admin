...
<el-row class="content-body" v-loading="loading" element-loading-text="拼命加载中">
  <el-row v-if="item != null"class="item-detail">
    <d-text label="上班提示语状态">
      <d-maps v-model="item.greetingIswork" :options="[{'value':0, 'label':'关闭'}, {'value':1, 'label':'开启'}]"></d-maps>
    </d-text>
    <d-text label="上班提示语">{{item.greetingWorktip}}</d-text>
    <d-text label="下班提示语状态">
      <d-maps v-model="item.greetingIsknockoff" :options="[{'value':0, 'label':'关闭'}, {'value':1, 'label':'开启'}]"></d-maps>
    </d-text>
