...
layui.define(['layer', 'laytpl', 'upload', 'layedit'], function(exports){
  ...
  layedit.set({
    hideTool: ['strong', 'italic', 'underline', 'del', '|', 'left', 'center', 'right', 'link', 'unlink', 'face', 'image', 'help' ],
    height: 100
  });  
  ...
  //关闭聊天窗口
  LAYIM.prototype.closeChat = function(){
    return closeChat(), this;
  };
  
  //插入聊天内容
  LAYIM.prototype.insert = function(content){
    var thatChat = thisChat();
    if(!thatChat) return;
    return focusInsert(thatChat.textarea[0], content);
  };
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" lay-accept="images"></span>'
        ,'{{# }; }}'
        ...
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="layim-chat-textarea"><textarea id="textarea_{{ d.data.id }}" class="textarea"></textarea></div>'
  ...
  layui.data.content = function(content){
    ...
    .replace(/face\[([^\s\[\]]+?)\]/g, function(face){  //转义表情
      ...
      return '<img class="layui-layim-faces" alt="'+ alt +'" title="'+ alt +'" src="' + faces[alt] + '">';
    })
    ...
    .replace(/file\([\s\S]+?\)\[[\s\S]*?\]/g, function(str){ //转义文件
      ...
      if(!href) return str;
      return '<a class="layui-layim-file" href="'+ href +'" download target="_blank" contenteditable="false"><i class="layui-icon">&#xe61e;</i><cite>'+ (text||href) +'</cite></a>';
    })
    .replace(/audio\[([^\s]+?)\]/g, function(audio){  //转义音频
      return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + audio.replace(/(^audio\[)|(\]$)/g, '') + '" contenteditable="false"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>';
    })
    .replace(/video\[([^\s]+?)\]/g, function(video){  //转义音频
      return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" data-src="' + video.replace(/(^video\[)|(\]$)/g, '') + '" contenteditable="false"><i class="layui-icon">&#xe652;</i></div>';
    })
  ...
  layui.data.code = function(code){
    //支持的ubb标签 HTML->UBB
    var ubb = function(end){
      return new RegExp('\<'+ (end||'') +'(code|pre|div|span|p|table|thead|th|tbody|tr|td|ul|li|ol|li|dl|dt|dd|h2|h3|h4|h5)([^>]*?)>', 'g');
    };
    code = (code||'').replace(/&amp;(?!#?[a-zA-Z0-9]+;)/g, '&')
    .replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&#39;/g, '\'').replace(/&quot;/g, '"') //XSS
    .replace(/@<a\shref="javascript:;">(\s+)<\/a>(\s+?|$)/g, '@$1$2') //转义@
    .replace(/<img\sclass="layui-layim-faces"\salt="\[([^\s\[\]]+?)\]"[^>]*?>/g, 'face[$1]')
    .replace(/<img[\s\S]+?src="([^\s]+?)"[^>]*?>/g, 'img[$1]')
    .replace(/<a\sclass="layui-layim-file"\shref="([\s\S]+?)"[^>]*?>[\s\S]+<cite>(.+)<\/cite><\/a>/g, 'file($1)[$2]') //未完成
    .replace(/<div\sclass="layui-unselect\slayui-layim-audio"\slayim-event="playAudio"\sdata-src="([^\s]+?)"[^>]*?><\/?[^>]*>([\s\S]*?)*<p>([\s\S]+?)<\/p><\/div>/g, 'audio[$1]')
    .replace(/<div\sclass="layui-layim-unselect\slayui-layim-video"\slayim-event="playVideo"\sdata-src="([^\s]+?)"[^>]*?><\/?[^>]*>([\s\S]*?)*<p>([\s\S]+?)<\/p><\/div>/g, 'video[$1]')
    
    .replace(/<a\shref="([\s\S]+?)"([^>]*?)>([\s\S]*?)<\/a>/g, 'a($1)[$3]')
    .replace(ubb(), '\[$1 $2\]').replace(ubb('/'), '\[/$1\]') //转移HTML代码
    .replace(/<br>/g, '\n') //转义换行 
    return code;
  };
  ...
  var layimMain, popim = function(content){
    ...
    //   ,success: function(layero){
        layimMain = layero = $('#layui-layim').html(content);
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
	...
	changeChat(list.find('.layim-chatlist-'+ data.type + data.id));
        var thatChat = thisChat();
        var editIndex = layedit.build('textarea_' + thatChat.data.id);
        thatChat.textarea.attr('data-index', editIndex);
    ...
    //   ,success: function(layero){
        layimChat = layero = $('#layui-layim-chat').html(laytpl('<ul class="layui-unselect layim-chat-list">'+ elemChatList +'</ul><div class="layim-chat-box">' + elemChatTpl + '</div>').render(render));;
        index = chatIndex = 0;
  ...
  var sendMessage = function(){
    ...
    var editIndex = Number(thatChat.textarea.attr('data-index'));
    data.content = layui.data.code(layedit.getContent(editIndex));
  ...
  var hotkeySend = function(){
    ...
    textarea.off('keydown').on('keydown', function(e){
      ...
      if(keyCode === 13){
        if(e.ctrlKey){
          return layedit.getContent(editIndex); //textarea.val(textarea.val()+'\n');
        }
  ...
  var focusInsert = function(obj, str){
    var result, val = obj, editIndex = Number($(obj).attr('data-index'));
    layedit.insertContent(editIndex, layui.data.content(str));
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,msgbox: function(){
      ...
      return events.msgbox.index = layer.open({
        ...
        ,resize: false
        ,content: cache.base.msgbox + '&id=' + cache.mine.id
      });
    }
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        ,content: cache.base.chatLog + '?id=' + thatChat.data.id + '&type=' + thatChat.data.type + '&mineId=' +cache.mine.id
      });
    }
